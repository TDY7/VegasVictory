...
$userScore = 0; // Score par défaut
if ($userEntry !== null) {
	$userScore = $userEntry[3];
}
...
// Vérifier si $joueurs est un tableau
if (!is_array($joueurs)) {
	echo "Erreur : le fichier joueurs.json ne contient pas de tableau valide.";
	exit();
}

// On filtre les joueurs actifs
$activeJoueurs = array_filter($joueurs, function($player) {
	return $player['valeur'] == 1; // Vérifiez si la valeur est égale à 1
});
...
<head>
	...
	<title>VegasBlackJack - JEU </title>
...
<body class="connexion">
	...
	<div id="infos-joueurs">
		<div id="infos-joueurs-title">
			<span style="color: #981b22; display: inline-block; width: 10px; background-color: #981b22; height: 3px;vertical-align: middle;"></span>
			JOUEURS
			<span style="color: #981b22; display: inline-block; width: 10px; background-color: #981b22; height: 3px;vertical-align: middle;"></span>
		</div>
		<div id="active-users" style="text-align: center; font-weight: bold; font-size: 24px;">
		</div>
	</div>
	...
	<div id="boutonsOptions">
		<div class="additional-buttons">
			...
			<form action="leave_game.php" method="post">
				<button id="quitButton" type="submit">QUITTER</button>
			</form>
		</div>
	</div>
...
document.getElementById('lancerPartie').addEventListener('click', function() {
    ...
    // On affiche les cartes du croupier avec un délai de 3 secondes
    setTimeout(function() {
...
$.ajax({
    ...
    error: function(xhr, status, error) {
        console.error('Erreur lors de la mise à jour du score : ' + error);
                ...
                // Afficher Défaite si le joueur a un total inférieur au croupier
                ...
                resultatElement.innerHTML = '<span class="res">ÉGALITÉ</span><img src="images/egal.png" alt="Égalité" class="egalite"><span class="restexte">Scores identiques !</span>';
            }
        }
...
document.getElementById('innerOval').innerHTML = '';
document.getElementById('innerOval').appendChild(resultatElement);
...
document.body.appendChild(restartButton);
    }, 2000); // Délai de 2 secondes avant d'afficher le résultat
}
...
function afficherCartes(cartes, elementId) {
    ...
    document.getElementById(elementId).innerHTML += html; // Ajout des images des cartes
}
...
document.querySelector('.tirer-image').addEventListener('click', function() {
    // Effectuer une requête AJAX pour tirer une nouvelle carte
    $.ajax({
        url: 'jeu.php',
        type: 'POST',
        data: { action: 'tirer' },
...
$(document).ready(function() {
    function updateActiveUsers() {
        $.ajax({
            ...
            success: function(data) {
                var usersHTML = '';
                // On parcourt les données pour construire la liste des utilisateurs actifs
                data.forEach(function(user) {
                    // On ajoute chaque utilisateur
                    usersHTML += '<div class="active-user" style="color: ' + user.color + '">' + user.username + '</div>';
                });
                // On met à jour l'affichage des utilisateurs actifs
    ...
    // On met à jour la liste des utilisateurs actifs
